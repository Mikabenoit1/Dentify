...
const downloadUserDocument = async (documentType) => {
  try {
    ...
    // Assurez-vous que cette URL correspond exactement à celle définie dans votre backend
    ...
    a.href = downloadUrl;
    a.download = documentType + '.pdf'; // Nommez le fichier téléchargé
...
const handleEdit = () => {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    if (file) {
      try {
        console.log("Photo à uploader:", file);
        ...
        console.log("Réponse de l'upload de photo:", response);
        
        // Mise à jour du profil
        if (response && response.photoUrl) {
          // Mettre à jour les deux états : editedProfile et profile
          setEditedProfile(prev => ({
            ...prev,
          ...
          throw new Error("URL de photo non reçue du serveur");
        }
      ...
      try {
        ...
        setEditedProfile({
          ...
          documents: {
            ...
            [documentType]: response.document.filename 
          }
        });
